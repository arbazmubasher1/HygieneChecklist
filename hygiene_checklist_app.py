...
def reset_checklist_state():
    """Clear all non-auth state, keep user logged in, and refresh this page."""
    preserved = {k: st.session_state[k] for k in PERSIST_KEYS if k in st.session_state}
    st.session_state.clear()
    st.session_state.update(preserved)
    # bump a nonce to force widget keys to be fresh
    st.session_state["nonce"] = str(int(time.time() * 1000))
    # route stays on checklist (no login bounce)
    st.session_state["page"] = "checklist"
    st.rerun()

def logout():
    for k in ("authenticated", "user_email", "page", "nonce"):
        if k in st.session_state:
            del st.session_state[k]
    st.rerun()

def _nonce():
    return st.session_state.get("nonce", "")
...
def render_checklist():
    ...
    # ---------- branch detection from email ----------
    email = st.session_state.get("user_email", "")
    email_prefix = email.split("@")[1].split(".")[0] if "@" in email else ""
    branch_mapping = {
        "dhap6": "DHA-P6",
        "dhacc": "DHA-CC",
    ...
    a, b = st.columns([1,1])
    with a:
        if st.button("🔄 New Checklist", key=f"new_checklist_top{nonce}"):
            reset_checklist_state()
    with b:
        if st.button("🚪 Log out", key=f"logout_top{nonce}"):
            logout()
    ...
    # ---------- photo upload (instant ImgBB) ----------
    st.subheader("🖼️ Upload Photos")

    def photo_upload_section(label, session_key):
        """Upload from device and instantly upload to ImgBB."""
        # Show success if already uploaded this run
        ...
        if uploaded is not None:
            try:
                ...
                if url:
                    ...
                    st.session_state[f"{session_key}_photo_url"] = url  # compatibility
                    st.success(f"{label} photo uploaded ✅")
                else:
    ...
    # ---------- reference image (optional, safe) ----------
    reference_image = None
    caption = ""
    try:
        if employee_type == "Crew" and gender == "Male" and os.path.exists("Crew_male.PNG"):
            reference_image = Image.open("Crew_male.PNG")
            caption = "Crew Male Appearance"
        elif employee_type == "Crew" and gender == "Female" and os.path.exists("Crew_female.PNG"):
            reference_image = Image.open("Crew_female.PNG")
            caption = "Crew Female Appearance"
        elif employee_type == "Rider" and os.path.exists("Rider_male.PNG"):
    ...
    if st.button("✅ Submit Checklist", key=f"submit{nonce}"):
        ...
        employee_url = (
            ...
            or st.session_state.get("employee_photo_url")
        )
        bike_url = (
        ...
        with col_a:
            if st.button("🔄 New Checklist", key=f"new_checklist_bottom{nonce}"):
                reset_checklist_state()
        with col_b:
            if st.button("🚪 Log out", key=f"logout_bottom{nonce}"):
                logout()
