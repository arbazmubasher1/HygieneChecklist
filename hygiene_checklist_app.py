...
date = st.date_input("🗕️ Date", value=datetime.today())
...
# --- Helper: Custom Checklist Button + Remark ---
...
all_fields = [*hygiene_fields.items()]
# Add conditional groups
for group in ["safety_checks", "documents_check", "bike_inspection"]:
    if group in locals():
        all_fields.extend(eval(group).items())

total_items = len(all_fields)
filled_items = sum(1 for _, v in all_fields if v["selection"] in ["✅", "❌", "✍"])
